...
async function generateSuggestion(prompt: string): Promise<string> {
  ...
  const preferredModel = process.env.OLLAMA_MODEL || "codellama:7b";
  const timeoutMs = Number(process.env.OLLAMA_TIMEOUT_MS || "0");
  const numPredict = Number(process.env.OLLAMA_NUM_PREDICT || "120");
  ...
  const requestBody = (model: string) => ({
    ...
    options: {
      temperature: 0.3,
      num_predict: Number.isFinite(numPredict) ? numPredict : 120,
    },
  });
  ...
  try {
    ...
    try {
      suggestion = await callOllama(preferredModel);
    } catch (preferredError) {
      console.warn(`Preferred model '${preferredModel}' failed, trying fallback model`, preferredError);
    }

    if (!suggestion) {
      const fallbackModel = await getFallbackModel();
      if (fallbackModel) {
        suggestion = await callOllama(fallbackModel);
      }
    }
    ...
    return suggestion || "// Start implementing here"
  } catch (error) {
      console.error("AI generation error:", error)
    return "// Start implementing here"
  }
}
